...
const Spinner = ({ colorClass }) => (
    <svg className={`animate-spin h-8 w-8 mb-4 ${colorClass}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
const Dashboard = () => {
    ...
    // Track which action is currently loading ('pull' | 'restart' | null)
    ...
    const executeScript = async (type) => {
        // Prevent execution if any action is already running
        if (loadingAction) return;

        setLoadingAction(type); // Start Loading
        ...
        try {
            ...
            setLoadingAction(null); // Stop Loading (even if error occurs)
        }
    };
        ...
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                ...
                <button
                    ...
                    className={`flex flex-col items-start p-6 rounded-2xl bg-slate-900 border border-slate-800 text-left shadow-lg transition-all 
                        ${loadingAction 
                            ? 'opacity-50 cursor-not-allowed' 
                            : 'hover:border-amber-500/50 hover:bg-slate-800/50 hover:shadow-amber-900/10 group'
                        }`}
                >
                    {loadingAction === 'pull' ? (
                        <Spinner colorClass="text-amber-500" />
                    ) : (
                    ...
                    <h3 className="text-lg font-semibold text-white group-hover:text-amber-400 transition-colors">
                        {loadingAction === 'pull' ? 'Pulling Changes...' : 'Pull Changes'}
                    </h3>
                ...
                <button
                    ...
                    className={`flex flex-col items-start p-6 rounded-2xl bg-slate-900 border border-slate-800 text-left shadow-lg transition-all 
                        ${loadingAction 
                            ? 'opacity-50 cursor-not-allowed' 
                            : 'hover:border-red-500/50 hover:bg-slate-800/50 hover:shadow-red-900/10 group'
                        }`}
                >
                    {loadingAction === 'restart' ? (
                        <Spinner colorClass="text-red-500" />
                    ) : (
                    ...
                    <h3 className="text-lg font-semibold text-white group-hover:text-red-400 transition-colors">
                        {loadingAction === 'restart' ? 'Restarting Service...' : 'Restart Service'}
                    </h3>
